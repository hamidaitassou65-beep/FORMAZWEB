...
import { X, Send, Paperclip, AlertCircle, CheckCircle2, ShieldCheck, FileText, Info } from 'lucide-react';
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[92vh] overflow-hidden flex flex-col border border-slate-200">
        ...
        {isSubmitted ? (
          <div className="p-8 text-center space-y-4 my-auto">
            ...
            <h4 className="text-2xl font-bold text-[#1a365d]">Demande enregistrée</h4>
            <p className="text-sm text-slate-600 max-w-md mx-auto leading-relaxed">
              Votre demande concernant <strong className="text-[#1a365d]">{formData.relatedSubject || formData.requestType}</strong> a été formulée avec succès.
            </p>
            
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 text-left text-xs text-slate-600 space-y-2 max-w-lg mx-auto">
              <div className="font-semibold text-[#1a365d] flex items-center gap-1.5">
                <Info className="w-4 h-4 text-orange-500" /> Information de configuration technique :
              </div>
              <p>
                L’interface de demande est opérationnelle. Pour activer la transmission automatique des e-mails vers votre messagerie, connectez un service SMTP ou une route API backend.
              </p>
            </div>

            <button
              onClick={onClose}
              className="mt-4 px-6 py-2.5 bg-[#1a365d] hover:bg-[#152c4d] text-white font-bold text-sm rounded-lg transition-colors"
            >
              Fermer la fenêtre
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-4 text-slate-800">
            ...
            <div className="pt-2">
              <label className="flex items-start gap-2.5 cursor-pointer text-xs text-slate-600">
                ...
                />
                <span>
                  J’accepte le traitement de mes données personnelles dans le cadre strict de l’étude de ma demande industrielle. <ShieldCheck className="w-3.5 h-3.5 inline text-slate-600 ml-1" />
                </span>
              </label>
            </div>
            ...
            <div className="pt-4 border-t border-slate-200 flex items-center justify-end gap-3">
              ...
              <button
                ...
                className="px-6 py-2.5 bg-orange-500 hover:bg-orange-600 text-white font-bold text-xs rounded-lg shadow-md flex items-center gap-2 disabled:opacity-50 transition-colors"
              >
                {isSubmitting ? (
